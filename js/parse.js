function $ParseProvider() {
    ...
    function markConstantAndWatchExpressions(ast, $filter) {
        ...
        switch (ast.type) {
            case AST.Program:
                allConstants = true; _.forEach(ast.body, function (expr) {
            ...
            case AST.ArrayExpression:
                ...
                ast.constant = allConstants; ast.toWatch = argsToWatch; break;
            ...
            case AST.ThisExpression: ast.constant = false; ast.toWatch = []; break;
            case AST.MemberExpression: markConstantAndWatchExpressions(ast.object, $filter);
                if (ast.computed) {
                    markConstantAndWatchExpressions(ast.property, $filter);
                }
                ast.constant = ast.object.constant &&
                    (!ast.computed || ast.property.constant);
                ast.toWatch = [ast]; break;
            case AST.CallExpression:
                ...
                ast.constant = allConstants;
                ast.toWatch = stateless ? argsToWatch : [ast]; break;
            ...
            case AST.ConditionalExpression:
                ...
                ast.constant =
                    ast.test.constant && ast.consequent.constant && ast.alternate.constant; ast.toWatch = [ast];
                break;
        }
    }
        ...
        while (this.index < this.text.length) {
            ...
            if (escape) {
                if (ch === 'u') {
                    ...
                    var replacement = ESCAPES[ch]; if (replacement) {
                        string += replacement;
                    } else {
                        string += ch;
                    }
    ...
    AST.prototype.constants = {
        'null': { type: AST.Literal, value: null }, 'true': { type: AST.Literal, value: true }, 'false': { type: AST.Literal, value: false }, 'this': { type: AST.ThisExpression }
    };
    ...
    ASTCompiler.prototype.watchFns = function () {
        ...
        _.forEach(this.state.inputs, function (inputName) {
            result.push('var ', inputName, '=function(s) {', (self.state[inputName].vars.length ?
    ...
    ASTCompiler.prototype.compile = function (text) {
        ...
        this.state = { body: [], nextId: 0, fn: { body: [], vars: [] }, filters: {}, assign: { body: [], vars: [] }, inputs: [] };
        this.stage = 'inputs';
        _.forEach(getInputs(ast.body), function (input, idx) {
            var inputKey = 'fn' + idx;
            self.state[inputKey] = { body: [], vars: [] };
            self.state.computing = inputKey; self.state[inputKey].body.push('return ' + self.recurse(input) + ';'); self.state.inputs.push(inputKey);
        });
        this.stage = 'assign';
        ...
        var fn = new Function('ensureSafeMemberName', 'ensureSafeObject', 'ensureSafeFunction', 'ifDefined',
            'filter',
            fnString)(
        ...
        switch (ast.type) {
            case AST.Program:
                _.forEach(_.initial(ast.body), function (stmt) {
                    self.state[self.state.computing].body.push(self.recurse(stmt), ';');
                });
                this.state[this.state.computing].body.push(
                    'return ', this.recurse(_.last(ast.body)), ';'); break;
            case AST.Literal:
                return this.escape(ast.value);
            case AST.ThisExpression: return 's';
            case AST.Identifier: ensureSafeMemberName(ast.name); intoId = this.nextId();
                var localsCheck;
                ...
                this.if_(this.not(localsCheck) + ' && s',
                    this.assign(intoId, this.nonComputedMember('s', ast.name))); if (context) {
                        context.context = localsCheck + '?l:s'; context.name = ast.name; context.computed = false;
                    } this.addEnsureSafeObject(intoId); return intoId;
            case AST.ObjectExpression:
            ...
            case AST.LogicalExpression:
                intoId = this.nextId(); this.state[this.state.computing].body.push(
                    this.assign(intoId, this.recurse(ast.left))); this.if_(ast.operator === '&&' ? intoId : this.not(intoId),
                        this.assign(intoId, this.recurse(ast.right))); return intoId;
            case AST.NGValueParameter: return 'v';
            case AST.ConditionalExpression:
                intoId = this.nextId();
                var testId = this.nextId(); this.state[this.state.computing].body.push(
                    this.assign(testId, this.recurse(ast.test))); this.if_(testId,
                        this.assign(intoId, this.recurse(ast.consequent))); this.if_(this.not(testId),
                            this.assign(intoId, this.recurse(ast.alternate))); return intoId;
        }
    };
